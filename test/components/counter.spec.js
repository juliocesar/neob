  ...
  describe('Counter', () => {
    ...
    it('should receive and increment counter', () => {
      ...
      TestUtils.Simulate.click(
        TestUtils.findRenderedDOMComponentWithTag(counter, 'button')
      )
      expect(counter.props.count).to.be.equal(1)
      expect(
        TestUtils.findRenderedDOMComponentWithTag(counter, 'h1').textContent
      ).to.contain('1')
    })
    ...
    describe('increment', () => {
      it('should get called when a click on button happens', () => {
        ...
        const counter = TestUtils.renderIntoDocument(
          <Counter onIncrement={spy} count={0} />
        )
